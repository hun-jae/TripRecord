<template>
  <b-container class="bv-example-row mt-3 text-left" style="width: 60%">
    <trip-search
      :isModal="isModal"
      :value="dialog"
      @input="dialog = $event"
    ></trip-search>
    <HotplaceSearch class="mb-5" @search="searchkeyword"></HotplaceSearch>
    <b-row class="mb-1">
      <b-col class="text-right">
        <v-btn color="rgba(0, 123, 255,0.2)" x-large @click.stop="dialog = true"
          >글쓰기</v-btn
        >
      </b-col>
    </b-row>
    <v-container fluid class="text-left" style="margin: 0 auto">
      <v-row dense class="text-left" style="margin: 0 auto">
        <v-col
          v-for="article in articles"
          :key="article.bid"
          :cols="12"
          md="4"
          class="text-left"
        >
          <v-card max-width="400" class="mb-5" @click="viewArticle(article)">
            <v-img
              v-if="article.imgSrc != null && article.imgSrc != ``"
              class="align-end text-white"
              :src="article.imgSrc"
              height="200px"
              cover
            ></v-img>
            <v-img
              v-else
              class="align-end text-white"
              src="@/assets/img/춘식.png"
              height="200px"
              cover
            ></v-img>
            <v-card-title>
              <template>
                <router-link
                  :to="{ name: 'hotplaceview', params: { bid: article.bid } }"
                  class="text-truncate body-2"
                  style="font-weight: 800; color: black"
                >
                  <div style="font-weight: 900; font-size: 1.2em; color: black">
                    {{ article.title }}
                  </div>
                </router-link>
              </template>
            </v-card-title>
            <v-card-subtitle>
              <span class="me-1">작성자 : {{ article.uid }}</span>
            </v-card-subtitle>
            <v-row>
              <v-col>
                <v-card-text class="body-2 text-truncate text-left" style="">
                  {{ article.content }}
                </v-card-text>
              </v-col>
            </v-row>
          </v-card>
        </v-col>
      </v-row>
    </v-container>
  </b-container>
</template>

<script>
import { listArticle, search } from "@/api/hotplace";
import HotplaceSearch from "../hotplace/HotplaceSearch.vue";
import TripSearch from "../TripSearch.vue";

export default {
  name: "HotplaceList",
  data() {
    return {
      from: null,
      articles: [],
      fields: [
        { key: "bid", label: "글번호", tdClass: "tdClass" },
        { key: "title", label: "제목", tdClass: "tdSubject" },
        { key: "uid", label: "작성자", tdClass: "tdClass" },
        { key: "regtime", label: "작성일", tdClass: "tdClass" },
        { key: "numHeart", label: "조회수", tdClass: "tdClass" },
      ],
      isModal: true,
      dialog: false,
    };
  },
  components: {
    HotplaceSearch,
    TripSearch,
  },
  async created() {
    // let from = this.$route.params.from;
    // this.from = from;
    let param = {
      pgno: 1,
      keyword: "",
      search_code: "",
    };
    await listArticle(
      param,
      ({ data }) => {
        console.log(data);
        this.articles = data;
      },
      (error) => {
        console.log(error);
      }
    );
  },
  methods: {
    moveWrite() {
      this.$router.push({ name: "hotplacewrite" });
    },
    viewArticle(article) {
      this.$router.push({
        name: "hotplaceview",
        params: { bid: article.bid },
      });
    },
    searchkeyword(keyword) {
      search(
        keyword,
        ({ data }) => {
          console.log(data);
          this.articles = data;
        },
        (error) => {
          console.log(error);
        }
      );
    },
  },
};
</script>

<style scope>
.tdClass {
  width: 50px;
  text-align: center;
}
.tdSubject {
  width: 300px;
  text-align: left;
}
</style>
